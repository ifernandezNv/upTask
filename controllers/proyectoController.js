...
const agregarColaboradores = async (req, res) => {
    ...
    // El colaborador agregado es el administrador del proyecto
    if(proyecto.creador.toString() === usuario._id.toString()){
        const error = new Error('El creador del proyecto no puede ser colaborador');
        return res.status(401).json({msg: error.message})
    }

    // Revisar que el colaborador no este agregado al proyecto
    if(proyecto.colaboradores.includes(usuario._id)){
        const error = new Error('El usuario ya pertenece al proyecto');
        return res.status(401).json({msg: error.message})
    }

    // Agregando el colaborador al proyecto
...
const eliminarColaborador = async (req, res) => {
    ...
    const {email} = req.body;
    const usuario = await Usuario.findOne({email}).select('-password -createdAt -confirmado -token -updatedAt -__v');
    // Eliminando el colaborador del proyecto
